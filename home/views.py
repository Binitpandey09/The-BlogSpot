...
def profile_view(request):
    ...
    posts_count = BlogPost.objects.filter(author=request.user).count()
    comments_count = Comment.objects.filter(user=request.user).count()
    latest_posts = BlogPost.objects.filter(author=request.user).order_by('-dateTime')[:5]
...
def edit_profile(request):
    ...
    return render(request, "edit_profile.html", {'form': form, 'profile': profile, 'user': request.user,})
